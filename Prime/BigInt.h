...
bool ggint_equal(const gint a, const gint b)
{
    size_t i=0;
...
// a < b
bool ggint_less(const gint a, const gint b)
{
    size_t i=0;
    for (i = GInt_Size - 1; ; i--)
    {
...
// a <= b
bool ggint_less_or_equal(const gint a, const gint b)
{
    size_t i=0;
    for (i = GInt_Size - 1; ; i--)
    {
...
// a == 0
bool ggint_is_zero(const gint a)
{
    size_t i=0;
    for (i = 0; i < GInt_Size; ++i)
...
// a & 1 == 0
bool ggint_is_even(const gint a)
{
    return (a[0] & 1) == 0;
}
// a & 1 == 1
bool ggint_is_odd(const gint a)
{
    return (a[0] & 1) == 1;
}
// a = 0
...
//b = b + a
void ggint_add_gint(const gint a, gint b)
{
    uint16_t x = 0;
    size_t i = 0;
    for (i = 0; i < GInt_Size; i++)
...
//b = b - a
void ggint_sub_gint(const gint a, gint b)
{
    uint16_t r = 0;
    size_t i = 0;
    for (i = 0; i < GInt_Size; i++)
...
void ggint_sub_int(const uint8_t a, gint b)
{
    uint16_t r = a;
    size_t i = 0;
    for (i = 0; i < GInt_Size; i++)
...
// b = b * a
void ggint_mul_int(const uint8_t a, gint b)
{
    uint16_t r = 0;
    size_t i = 0;
    for (i = 0; i < GInt_Size; ++i)
...
// p = a * b
void ggint_mul_gint(const gint a, const gint b, gint p)
{
    gint t;
    ggint_zero(p);
    size_t i=0;
...
// b / a = q, b % a = r
void ggint_div_gint(const gint a, const gint b, gint q, gint r)
{
    ggint_zero(q);
    ggint_zero(r);
    gint t;
...
// b % a = r
void ggint_mod_gint2(const gint a, const gint b, gint r)
{
    gint q;
    ggint_div_gint(a,b,q,r);
}
void ggint_mod_gint(const gint a, const gint b, gint r)
{
    if (ggint_less(b, a))
    {
        ggint_set_gint(r,b);
...
// b % a = r
void ggint_mod_int(size_t a, const gint b, size_t *r)
{
    *r = 0;
    size_t i = 0;
    for (i = GInt_Size - 1;; --i)
...
void ggint_rand_range(gint a, const gint b)
{
    ggint_rand(a);
    gint t;
    ggint_set_gint(t, a);
...
// r = a^x mod n
void ggint_pow_mod(gint a, gint x, const gint n, gint r)
{
    gint t;
    ggint_one(r);
    ggint_zero(t);
